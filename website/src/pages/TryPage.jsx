...
export default function TryPage() {
  ...
  return <main className="min-h-screen bg-[#fffaf2] px-4 pb-20 pt-24">
    <div className="mx-auto max-w-6xl">
      ...
      {tab === 'browser' ? <>
        <section className="mt-10 grid overflow-hidden rounded-[32px] border border-amber-100 bg-white shadow-xl shadow-amber-900/5 lg:grid-cols-[1.45fr_.85fr]">
          <div className="bg-[#07110f] p-3 sm:p-5"><iframe title="VaaniSeva live browser voice call" src={`${VOICE_BASE}/local?embed=product`} allow="microphone" className="h-[420px] w-full rounded-3xl border-0" /></div>
          <div className="flex flex-col justify-between gap-5 p-6 sm:p-8"><div><div className="flex items-center gap-2 text-sm font-semibold text-emerald-700"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Production voice pipeline</div><h2 className="mt-4 text-2xl font-bold text-content-primary">Speak naturally. Interrupt anytime.</h2><p className="mt-3 text-sm leading-relaxed text-content-secondary">Allow microphone access, then ask in Hindi or English. The browser uses the same STT → Bedrock → tools → TTS architecture as the telephone channel.</p></div><Suspense fallback={<div className="h-40 animate-pulse rounded-2xl bg-amber-50" />}><AvatarShowcase /></Suspense></div>
        </section>
